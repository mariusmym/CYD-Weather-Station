...
A weather station firmware for the **ESP32-2432S028R** (Cheap Yellow Display) using the free OpenWeather API.

Hardware reference: [ESP32-Cheap-Yellow-Display](https://github.com/witnessmenow/ESP32-Cheap-Yellow-Display)


The image below is **just a mockup** for the first two tabs, based on the dark theme colors and layout constants from the code. The actual TFT rendering will look slightly different due to the bitmap fonts and pixel-level drawing, but the layout, colors and proportions should be close. More real life pictures in the images folder.
